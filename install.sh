...
GITEA_SERVER_PORT="${GITEA_SERVER_PORT:-3000}"
GITEA_SERVER_HOST="${GITEA_SERVER_HOST:-$(hostname -f 2>/dev/null)}"
...
if [ -f "$INSTDIR/docker-compose.yml" ] && cmd_exists docker-compose; then
  ...
  if docker ps -a | grep -qsw "$APPNAME"; then
    ...
    __sudo docker run -d \
      ...
      --privileged \
      -e TZ="$GITEA_SERVER_TIMEZONE" \
      -v "$DATADIR/data":/data:z \
      -v "$DATADIR/config":/etc/gitea:z \
      -p 7822:7822 \
      -p "$GITEA_SERVER_PORT":3000 \
      "$DOCKER_HUB_URL" &>/dev/null
  fi
fi
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
if docker ps -a | grep -qs "$APPNAME"; then
  printf_blue "Service is available at: http://$GITEA_SERVER_HOST:$GITEA_SERVER_PORT"
else
  false
fi
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
